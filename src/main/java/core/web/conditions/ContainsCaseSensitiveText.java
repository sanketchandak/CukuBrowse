...
public class ContainsCaseSensitiveText extends Condition {
    ...
    public ContainsCaseSensitiveText(By elementBy, String elementText, boolean shouldBe) {
        this.elementBy = elementBy;
        this.elementText = elementText;
        this.shouldBe = shouldBe;
    }

    public ContainsCaseSensitiveText(WebElement element, String elementText, boolean shouldBe) {
        ...
        try {
            return wait.until(d -> {
                        try {
                            ...
                            return shouldBe == uiElementText.get().contains(elementText);
            ...
            throw new TimeoutException(String.format("Sensitive text '%s' to be present in element '%s'. Actual text is '%s'.", elementText, elementBy != null ? elementBy : element.toString(), uiElementText.get()),
                    timeoutException);
        }
    }
}
