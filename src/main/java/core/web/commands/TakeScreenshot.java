...
public class TakeScreenshot {
    ...
    public byte[] takeScreenshotAsBytes(WebElement... element) {
        assert element != null;
        setDriverSession();
        logger.info(String.format("Take Screenshot As Bytes: take screenshot of element '%s' as BYTES", (element.length != 0) ? Arrays.stream(element).map(Object::toString).collect(Collectors.toList()) : ""));
        return element.length == 0 ?
                ((TakesScreenshot) driver).getScreenshotAs(OutputType.BYTES) :
                element[0].getScreenshotAs(OutputType.BYTES);
    }
